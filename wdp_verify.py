...
class WDPVerifier:
    ...
    def parse_wdp_markers(self, code: str) -> List[WDPBlock]:
        ...
        while i < len(lines):
            line = lines[i]
            # Match start marker: # @wd: BLOCK_TYPE | module-id=XXX | ...
            match = re.match(r'#\s*@wd:\s*(FREEZE|GUARD|AUDIT)\s*\|', line)
            if match:
                ...
                j = i + 1
                while j < len(lines):
                    if '# @wd: END' in lines[j]:
                        end_line = j + 1
                        break
                    content_lines.append(lines[j])
                    j += 1
                else:
                    # No end marker found, treat as error
                    end_line = len(lines)
                ...
                # Extract signature lock (if present)
    ...
    def verify(self, original_code: str, modified_code: str) -> List[Violation]:
        ...
        for orig_block in original_blocks:
            ...
            if not found_block:
                ...
                self.violations.append(Violation(
                    severity="CRITICAL",
                    message=f"保护块被删除：{orig_block.block_type} Module {orig_block.module_id}",
            ...
            if orig_block.content_hash != found_block.content_hash:
                if orig_block.block_type == "FREEZE":
                    self.violations.append(Violation(
                        severity="CRITICAL",
                        message=f"FREEZE block content tampered: Module {orig_block.module_id} (hash changed:{orig_block.content_hash} -> {found_block.content_hash})",
                    ...
                    self.violations.append(Violation(
                        severity="WARNING",
                        message=f"GUARD block content modified: Module {orig_block.module_id}",
                        line_number=found_block.start_line,
                        module_id=orig_block.module_id
                    ))
                    ...
                    self.violations.append(Violation(
                        severity="INFO",
                        message=f"AUDIT block content modified: Module {orig_block.module_id} (requires audit)",
                        line_number=found_block.start_line,
                        module_id=orig_block.module_id
                    ))
            ...
            if orig_block.signature_lock and orig_block.signature_lock != found_block.signature_lock:
                self.violations.append(Violation(
                    severity="CRITICAL",
                    message=f"签名锁验证失败：Module {orig_block.module_id}",
                    line_number=found_block.start_line,
                    module_id=orig_block.module_id
                ))
        ...
        for mod_block in modified_blocks:
            if mod_block.module_id not in orig_module_ids:
                self.violations.append(Violation(
                    severity="WARNING",
                    message=f"检测到新的保护块注入：{mod_block.block_type} Module {mod_block.module_id}",
    ...
    def generate_report(self) -> str:
        ...
        if not self.violations:
            return "✅ No violations detected. Code complies with WDP protocol."
        
        report = ["❌ WDP protocol violations detected:\n"]
        ...
        report.append(f"Stats: {critical_count}  CRITICAL | {warning_count}  WARNING | {info_count} INFO\n")
        report.append("-" * 60 + "\n")
        
        for v in sorted(self.violations, key=lambda x: {"CRITICAL": 0, "WARNING": 1, "INFO": 2}[x.severity]):
            report.append(f"[{v.severity}] Line {v.line_number} | Module {v.module_id}\n")
